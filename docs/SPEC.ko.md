...
1. IANA delegated-TLD list의 ASCII 소문자 entry
2. infrastructure TLD (`arpa`) 가 아님
3. 비공개 TLD (`edu`, `gov`, `int`, `mil`, `va`) 가 아님
4. ICANN registry agreements CSV에서 `Brand (Spec 13)` 로 표시되지 않음
5. ICANN registry agreements CSV에서 상태가 `active` 가 아님으로 표시되지 않음
6. 공개 등록 probe 통과

ICANN registry agreements CSV에 없는 row는 이 단계에서 제외하지 않습니다.

비공개 TLD 목록의 구성:

- `edu`, `gov`, `int`, `mil`: IANA Root Zone Database (`type == "sponsored"`)와
  ICANN registry agreements CSV (active 항목 없음)의 교차 참조로 도출.
- `va`: Vatican City 전용으로 공개 registrar를 통한 등록 불가.
...
1. IANA delegated TLD list fetch
2. ICANN registry agreements CSV fetch 후 brand/non-active gTLD 제외
3. 공용 resolver 엔진으로 공개 등록 가능 여부 probe
4. TLD 정렬 및 그룹 분할
5. GitHub Actions matrix용 JSON 출력
